<?php
	...
	class Jungle_WikiSyntax_Parser {
		...
		private function tables($section_header, $section_syntax) {
			...
			preg_match_all("#(?:\{\||\{\{s\-start\}\})(.+?)(?:\|\}|\{\{s\-end\}\})\s+#si", $section_syntax, $matches);
			...
			*/
			...
			foreach($matches[0] as $mkey => $raw_table_full_syntax) {
				...
				$raw_table_inner_syntax = $matches[1][$mkey];
				
				if(preg_match("#\{{2}\s*Succession box#si", $raw_table_inner_syntax)) {
					// succession boxes... not parsed atm
					continue;
				}
				...
				foreach($table_lines as $tln => $table_line) {
					...
					$table_line = preg_replace("#(\|)?\s*(width|height|align|style|border|bgcolor|colspan|rowspan)=\"([^\"]+?)\"#si", "\\1", $table_line);
					$table_line = preg_replace("#(\|)?\s*(width|height|align|style|border|bgcolor|colspan|rowspan)=\'([^\']+?)\'#si", "\\1", $table_line);
					$table_line = preg_replace("#(\|)?\s*(width|height|align|style|border|bgcolor|colspan|rowspan)=([A-Za-z0-9\-\_\;\:\$\#\@\!\%\&\*\(\)]+)#si", "\\1", $table_line);
					
					$table_line = trim($table_line);
					
					$table_line = preg_replace("#^\|{2,}#si", "|", $table_line);
					
					$table_line = trim($table_line);
					
					$table_line = $this->clean_wiki_text($table_line, true);
					...
					if(preg_match("#^!(.+?)$#si", $table_line, $match)) {
						...
						foreach($head_cols as $header_column) {
				...
				if(!empty($rows)) {
					...
					$table['section'] = trim($this->clean_wiki_text($section_header));
		...
		private function external_links() {
			...
			foreach($lines as $line) {
				...
				switch(true) {
					...
					// {dmoz|xxx|yyy}
					case preg_match("#\{\{\s*(dmoz|yahoo directory)\s*\|([^\|]+?)\|([^\}\}]+?)\}\}#si", $line_value, $lmatch):
						$external_link = array(
							 'type' => $this->compact_to_slug($lmatch[1])
							,'attributes' => array(
								 'id' => trim($lmatch[2])
					...
					case preg_match("#\{{2}\s*(tv\.com|rotten\-tomatoes)\s*\|([^\|]+?)\|([^\}]+?)\}{2}#si", $line_value, $lmatch):
						...
						break;
					case preg_match("#\{\{\s*(allmusic|YouTube|[A-Za-z0-9\-\_]+?)\s*\|([^\}\}]+?)\}\}#si", $line_value, $lmatch):
						$tmp = array(
							 'type' => $this->compact_to_slug($lmatch[1])
							,'attributes' => array()
						);
						...
						if(!empty($tmp_attrs)) {
							...
							foreach($tmp_attrs as $tmp_value_raw) {
								...
								if(count($tmp_value) === 2) {
									$tmp['attributes'][ $this->compact_to_slug($tmp_value[0]) ] = trim( $tmp_value[1] );
								} else {
									$tmp['attributes'][] = trim($tmp_value[0]);
								}
							}
		...
		private function categories() {
			...
			if(!empty($matches[0])) {
				foreach($matches[1] as $nil => $mvalue) {
					$categories[] = trim(array_shift(explode("|", trim(trim(trim($mvalue), "|")))));
				}
			}
		...
		private function initial_clean() {
			...
			$this->text = str_replace("–", "-", $this->text);   // dashes
		...
		public function clean_wiki_text($wiki_text, $tags_too=false, $br_to_space=false) {
			...
			$wiki_text = preg_replace_callback("/\[{2}((?>[^\[\]]+)|(?R))*\]{2}/x", function($match) {
				if(preg_match("#^\[{2}\s*[A-Za-z]+\s*\:#si", $match[0])) {
					return "";
				}
		...
		private function finalize_internal_links($value) {
			...
			return preg_replace_callback("#\[{2}\s*([^\]]+?)\s*\]{2}#si", function($match) {
				...
				if(preg_match("#^([^\#]+?)\#([^\|]+?)\|(.+?)$#si", $match[1], $submatch)) {
					return "[wiki=". md5($submatch[1]) ."#". trim($submatch[2]) ."]". trim(preg_replace("#('{2,})(.+?)\\1#si", "\\2", $submatch[3])) ."[/wiki]";
				}
				
				if(preg_match("#^([^\#]+?)\|(.+?)$#si", $match[1], $submatch)) {
					return "[wiki=". md5(trim($submatch[1])) ."]". trim(preg_replace("#('{2,})(.+?)\\1#si", "\\2", $submatch[2])) ."[/wiki]";
				}
				
				if(preg_match("#^([^\#]+?)\#(.+?)$#si", $match[1], $submatch)) {
					return "[wiki=". md5(trim($submatch[1])) ."]". trim($submatch[1]) ." (". trim(preg_replace("#('{2,})(.+?)\\1#si", "\\2", $submatch[2])) .")[/wiki]";
				}
				
				return "[wiki=". md5(trim($match[1])) ."]". trim(preg_replace("#('{2,})(.+?)\\1#si", "\\2", $match[1])) ."[/wiki]";
			}, $value);
		}
	}
